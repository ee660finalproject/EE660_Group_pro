...
X_Train = np.load("X_ALL_10270.npy")
...
X_Train_Fin = X_Train[:,75:1270];
X_Train_UPC = X_Train[:,1270:];

X_Test_week = X_Test[:,0:7];
X_Test_Des = X_Test[:,7:75];
X_Test_Fin = X_Test[:,75:1270];
X_Test_UPC = X_Test[:,1270:];
...
nc =nc*2
lda = LinearDiscriminantAnalysis(n_components=nc).fit(X_Train_Fin,Y);
X_Train_Fin_lda= lda.transform(X_Train_Fin)
X_Test_Fin_lda = lda.transform(X_Test_Fin)

nc =nc*2
lda = LinearDiscriminantAnalysis(n_components=nc).fit(X_Train_UPC,Y);
X_Train_UPC_lda= lda.transform(X_Train_UPC)
X_Test_UPC_lda = lda.transform(X_Test_UPC)


X_Train_in = np.hstack([X_Train_week,X_Train_Des,X_Train_Fin,X_Train_UPC])
X_Test_in = np.hstack([X_Test_week,X_Test_Des,X_Test_Fin,X_Test_UPC])
