...
print("Extracting the top %d eigenfaces from %d faces"
      % (nc, X_train_fin.shape[0]))
t0 = time()
pca = RandomizedPCA(n_components=nc).fit(X_train_fin)
#print(pca.explained_variance_ratio_) 

kpca = RandomizedPCA(n_components=nc )
...
t0 = time()
X_train_pca = pca.transform(X_train_fin)
X_test_pca = pca.transform(X_test_fin)
np.save('X_train_pca_rd',X_train_pca)
np.save('X_test_pca_rd',X_test_pca)
print("done in %0.3fs" % (time() - t0))
