...
pre = classif.predict(X_val_fin)
score = accuracy_score(pre, Y_val.ravel())
print(score)
'''
...
t0 = time()
classif = RandomForestClassifier( n_estimators=500,max_depth=16);
classif.fit(X_train_pca, Y_train.ravel())

# Now predict the value of the digit on the second half:
pre = classif.predict(X_test_pca)
score = accuracy_score(pre, Y_test.ravel())
print(score)
